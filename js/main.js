...
function addItemsToLocalStorage() {
    var tempObj,
        sObj,
        condition = localStorage.getItem(0) === "data is loaded";
...
function outputALL(startPage) {
    ...
    currentPage = !startPage ? 1 : startPage;
    var size = currentTableSize || 10,
        paginationStopFlag = 0,
        startPosition = (startPage - 1) * currentTableSize || 0;

    for (var i = startPosition, l = workingSnippetStorage.length; i < l; i++) {
        if (size == paginationStopFlag) {
            break;
        }
        drawTable(i);
        paginationStopFlag++;
